...
const AdminRegister = () => {
  ...
  const handleOnSubmit = async (event) => {
    ...
    const allFieldsEntered = Object.keys(data).every(
      (key) => data[key].trim() !== ""
    );
    if (!allFieldsEntered) {
    ...
    try {
      console.log({ state });
      const { data } = await axios.post(`${BASE_API_URL}/adminregister`, {
        ...state,
      });
      console.log({ data });
      setSuccesMsg(`${state.accountType} registered successfully.`);
      setErrorMsg("");
      setTimeout(() => {
        setSuccesMsg("");
      }, 3000);
    ...
    <div>
      ...
      <div className="main-content">
        ...
        <Form onSubmit={handleOnSubmit} className="register-form">
          <h2>Doctor/Pharmacist Register</h2>
          ...
          <div className="first-form">
            ...
            <Form.Group>
              <div>
                If you have an account <Link to="/adminlogin">Click this</Link>
              </div>
              <Button
                variant="secondary"
                type="button"
